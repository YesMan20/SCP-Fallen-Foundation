...
public class SledgeHammerItem extends PickaxeItem {
    ...
    @Override
    public boolean hurtEnemy(ItemStack pStack, LivingEntity pEntity, LivingEntity pAttacker) {
        DamageSource sledgehammer = new FFDamageSources(pAttacker.level().registryAccess()).sledgehammer();
        pEntity.hurt(sledgehammer, 1.0F);
        return super.hurtEnemy(pStack, pEntity, pAttacker);
    }
